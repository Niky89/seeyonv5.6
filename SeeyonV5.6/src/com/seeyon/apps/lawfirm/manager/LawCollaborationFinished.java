...
public class LawCollaborationFinished
{
  ...
  public void onLawFinishedEvent(CollaborationFinishEvent event)
    throws Exception
  {
    try
    {
      ...
      else if (StringHelper.containBySp(formJa, templateNo, ",").booleanValue())
      {
        ...
        for (int i = 0; i < formDataMasterBean.size(); i++)
        {
            ...
            (ajbhInBaseino.equals(ajbhInFlow)))
          {
            ...
            FormDataMasterBean masterBean2 = FormService.findDataById(((FormDataMasterBean)formDataMasterBean.get(i)).getId().longValue(), ajxxFormId);
            Object oAjbh = masterBean2.getAllDataMap().get(((FormFieldBean)formFieldMap.get("公用-案件编号")).getName());
            if (oAjbh == null) {
              return;
            }
            ...
            String sZbls = "";
            if (masterBean2.getFieldValue(((FormFieldBean)formFieldMap.get("公用-主办律师")).getName()) != null) {
              sZbls = masterBean2.getFieldValue(((FormFieldBean)formFieldMap.get("公用-主办律师")).getName()).toString();
            }
            List cyryList = masterBean2.getDataList(((FormFieldBean)formFieldMap.get("公用-参与承揽与经办人员及律师费分配-人员")).getName());
            List xzryList = masterBean2.getDataList(((FormFieldBean)formFieldMap.get("公用-协助人员重复表-姓名")).getName());
            int zblsCount = 0;
            ...
            Long fid2 = LawDocHelper.findFolderByFullPath(fullPath2, null, null, null, null, null, null, null);
            if (fid2 == null || fid2==-1 || fid2==0) {
              break;
            }
            LawDocHelper.addLawGrant(fid2, Constants.ACCOUNT_LIB_TYPE.byteValue(), caseInfoAllMembers, zblsCount, Boolean.valueOf(true));
...
//        if ((objAjbh != null) && (!StringHelper.isNullOrEmpty(objAjbh.toString()).booleanValue())) {
//          return;
//        }
        ...
        String newAjbh = objAjbh.toString();//this.lawDataListManager.getAjbh(dq, lb, lasj);
//        try
//        {
//          log.info("ColFinished.....00008 " + CurrentUser.get().getName() + "," + templateNo + "," + summary.getSubject() + "," + newAjbh);
//        }
//        catch (Exception ex)
//        {
//          log.error("", ex);
...
//        masterBean.addFieldValue(((FormFieldBean)flowFieldMap.get("案件编号")).getName(), newAjbh);
        ...
        try
        {
          ...
          if (!isDuplicate.booleanValue())
          {
            ...
            LawDocHelper.addLawGrant(dr.getId(), Constants.ACCOUNT_LIB_TYPE.byteValue(), caseInfoAllMembers, zblsCount, Boolean.valueOf(false));
